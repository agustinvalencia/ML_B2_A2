...
```{r q1_time_series_plotting, fig.align='center', out.height='22%', q1=TRUE}
...
It can be seen that given the variability of mortality per week each year is the GAM fit acceptably well, though still not a good predictor.
...
```{r q1_penalty_factor_analysis, fig.align='center', out.height='70%', q1=TRUE}
...
ggplot(df) +
    ...
    geom_line(aes(x=time, y=sp_1u,  color="sp = 1u"), size=1) +
    geom_line(aes(x=time, y=sp_1m,  color="sp = 1m"), size=1) +
    geom_line(aes(x=time, y=sp_1 ,  color="sp = 1" ), size=1) +
    geom_line(aes(x=time, y=sp_1k,  color="sp = 1k"), size=1) +
    geom_line(aes(x=time, y=sp_1M,  color="sp = 1M"), size=1) +
...
At a first glance Residuals seems to be noise compared to Influenza behavior.

```{r q1_residuals_vs_Influenza, fig.align='center', out.height='40%', q1=TRUE}
...
Though, let's test numerically by getting their correlation statistic. 
...
Their correlation is relatively low, thus there is no evidence to say that the residuals from the model and the Influenza measures are correlated.
...
```{r q1_gam2_predictions, fig.align='center', out.height='40%', q1=TRUE}
...
splitData <- function(data, trainRate) {
    ...
    test <- data[-idxs,]
    print("Dividing data - ")
    cat("size of Total data :", nrow(train), "\n")
    cat("size of training   :", nrow(train), "\n")
    cat("size of testing    :", nrow(test), "\n")
...
```{r q2_train_crossvalidation_results, q2 = TRUE}
...
Therefore from the crossvalidation results it can be seen that the best performance error-wise is obtained for threshold = 1.65
...
# Elastic-net
#elasticnetModel <- glmnet(x = x, y = y, family="binomial", alpha=.5)
...
# SVM
svmModel <- svm(formula = Conference ~ . , data = train)
